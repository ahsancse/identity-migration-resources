...
CREATE OR REPLACE FUNCTION alter_idn_config_file_add_name() RETURNS void AS $$ BEGIN IF EXISTS (SELECT column_name FROM information_schema.columns WHERE  table_name='idn_config_file' and column_name='name') THEN RAISE NOTICE 'IDN Config file name Column already exists'; ELSE ALTER TABLE IF EXISTS IDN_CONFIG_FILE ADD COLUMN NAME VARCHAR(255) NULL;END IF;END;$$ LANGUAGE plpgsql;

select  alter_idn_config_file_add_name();
